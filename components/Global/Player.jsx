...
const Player = ({
  ...
  const onNextClick = (allPost) => {
    ...
    if (currentIndex < allPost.length - 1) {
    ...
    const nextMusic = allPost[nextIndex];
    setPlayMusic(nextMusic);

    // Maintain the progress of the song
    setPercentage(0);
    setCurrentTime(0);
    setSkiped(false);
  ...
  const onPreviousClick = (allPost) => {
    ...
    if (currentIndex > 0) {
      ...
      previousIndex = allPost.length - 1;
    }

    setCurrentIndex(previousIndex);

    const previousMusic = allPost[previousIndex];
    setPlayMusic(previousMusic);
              ...
              <div className="font-base text-base-xs font-medium text-base500">
                <a>
                  <span className="inline-block ">
                    {playMusic?.user.username || "Piyush Kotadiya"}
                  </span>
                </a>
              </div>
            </div>
