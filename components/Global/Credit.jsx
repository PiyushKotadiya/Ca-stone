...
const Credit = ({ setCredit, activeUser, setLoader, transferToOwnerAcc }) => {
  ...
  // CHECK_AUTH function to verify and fetch the current user
  ...
  // VERIFY_ACCOUNT function to handle account verification
  const VERIFY_ACCOUNT = async (VERIFY_AMOUNT) => {
    try {
      ...
      if (transaction && transaction.hash) {
        ...
        throw new Error("Transaction failed or returned undefined");
      }
    } catch (err) {
      console.log(err);
      setLoader(false);
  ...
  // BUY_CREDIT function to handle credit purchases
  const BUY_CREDIT = async (CREDIT_AMOUNT) => {
    try {
      const transaction = await transferToOwnerAcc(CREDIT_AMOUNT);
      const currentUser = await CHECK_AUTH();
      console.log(transaction);
      ...
      if (transaction && transaction.hash) {
        const res = await axios({
          ...
          withCredentials: true,
          data: { credit: Number(activeUser?.credit) + 50 },
        });
  ...
  return (
    <div className="c-kZStZF c-kZStZF-ihcRhWX-css" style={{ pointerEvents: "auto" }}>
      <div className="c-gLOVln c-gLOVln-cewiJJ-size-small" tabIndex={-1} style={{ pointerEvents: "auto" }}>
        <div className="z-above1 bg-white">
          <div className="flex justify-between">
            ...
            <button onClick={() => setCredit(false)} className="c-ebvTKE c-ebvTKE-isdEXf-variant-primary c-ebvTKE-ibJzEHE-css">
              <Collect1 />
            </button>
          </div>
        </div>
              ...
              <div className="flex w-full flex-col rounded-md lt:max-w-sm">
                <div className="z-above1 flex w-full flex-col gap-4 bg-white pt-4">
                          ...
                          <div className="flex flex-row items-start justify-between">
                            <div className="flex flex-col gap-1">
                              ...
                              <p className="font-base text-base-m font-medium text-base500">
                                Get your account verified and receive a verification badge
                              </p>
                            </div>
                          </div>
                  ...
                  <div className="flex justify-center">
                    <button
                      onClick={() => BUY_CREDIT(CREDIT_AMOUNT)}
